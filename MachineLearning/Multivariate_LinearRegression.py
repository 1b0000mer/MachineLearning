...
###input
filePath = os.getcwd() + '\multiLR\data.txt'
feature = ['Size', 'Room', 'Price']
training_data = pd.read_csv(filePath, names=feature)

#for plot
x_pop = np.linspace(training_data.values[:,0].min(), training_data.values[:,0].max(), 100,dtype=int)
y_pop = np.linspace(training_data.values[:,1].min(), training_data.values[:,1].max(), 100,dtype=int)
######
